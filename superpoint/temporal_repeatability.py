...
def create_kp_map(keypoints, image_size):
    print(image_size)
    kp_maps = np.zeros((int(image_size[0]/2), int(image_size[1]/2), len(keypoints)))
    for i in range(len(keypoints)):
        for kp1 in keypoints[i]:
            kp_maps[int(kp1.pt[1]/2), int(kp1.pt[0]/2), i] = 1
    return kp_maps

# def cal_repeatibility_score(keypoint_maps, window_size):
#
#     count = np.zeros(int(np.shape(keypoint_maps)[2]))
#     pad = int(np.floor(window_size/2))
#     # print("pad ", pad, np.shape(keypoint_maps)[2]-pad)
#     for i in range(int(pad), int(np.shape(keypoint_maps)[2]-pad)):
#         kp_maps = []
#         repeat = np.ones_like(np.squeeze(keypoint_maps[:, :, 0]))
#         count_total = 0
#         for j in [-pad, 0, pad]:
#             map = np.squeeze(keypoint_maps[:, :, i+j])
#             print("map shape", map.shape)
#             repeat = (repeat==1) & (map==1)
#             count_total += np.count_nonzero(map)
#             print("count repeat, total", np.count_nonzero(repeat), count_total)
#             kp_maps.append(map)
#
#         print("count repeat, total", i, np.count_nonzero(repeat), count_total)
#         repeatability_score = (np.count_nonzero(repeat) * 3) / count_total
#         count[i] = repeatability_score
#         # for kp1 in keypoints[0]:
#         #     for kp2 in keypoints[i]:
#         #         if(cal_kp_dist(kp1, kp2) < d_threshold and cal_angle_diff(kp1, kp2)):
#         #             count[i] += 1
#         #             break
#     return count
...
def cal_repeatibility_score(keypoint_maps, window_size):
    ...
    # pad = int(np.floor(window_size/2))
    # print("pad ", pad, np.shape(keypoint_maps)[2]-pad)
    for i in range(int(np.shape(keypoint_maps)[2]-window_size)):
        # kp_maps = []
        repeat = np.squeeze(keypoint_maps[:, :, i])
        count_total = np.count_nonzero(repeat)
        map = np.squeeze(keypoint_maps[:, :, i+window_size])
        print("map shape", map.shape)
        repeat = (repeat == 1) & (map == 1)
        # count_total += np.count_nonzero(map)
        # print("count repeat, total", np.count_nonzero(repeat), count_total)
        ...
        print("count repeat, total", i, np.count_nonzero(repeat), count_total)
        repeatability_score = np.count_nonzero(repeat) / count_total
        rep = rep + repeatability_score
        j = j + 1
        count[i] = repeatability_score
        # for kp1 in keypoints[0]:
        #     for kp2 in keypoints[i]:
...
if __name__ == '__main__':
    ...
    kp_folder = "/home/thamilchelvan.a/NUFRL/results/kp_mod_log/"
    ...
    kp_maps_SIFT = create_kp_map(kp_SIFT, [np.shape(images_orig)[1], np.shape(images_orig)[2]])
    kp_maps_SP = create_kp_map(kp_SP, [np.shape(images_orig)[1], np.shape(images_orig)[2]])
    kp_maps_SIFTmod = create_kp_map(kp_SIFTmod, [np.shape(images_orig)[1], np.shape(images_orig)[2]])
    kp_maps_IP = create_kp_map(kp_IP, [np.shape(images_orig)[1], np.shape(images_orig)[2]])

    window = 16
    cnt_SIFT, rep_SIFT = cal_repeatibility_score(kp_maps_SIFT, window*2)
    cnt_SP, rep_SP = cal_repeatibility_score(kp_maps_SP, window*2)
    cnt_SIFTmod, rep_SIFTmod = cal_repeatibility_score(kp_maps_SIFTmod, window*2)
    cnt_IP, rep_IP = cal_repeatibility_score(kp_maps_IP, window*2)

    fig, ax = plt.subplots(1)

    print("repeatability score SIFT: ", rep_SIFT)
    print("repeatability score mSIFT: ", rep_SIFTmod)
    print("repeatability score SuperPoint: ", rep_SP)
    print("repeatability score IRPoint: ", rep_IP)

    plt.plot(temporal_gap, cnt_SIFT)
    plt.plot(temporal_gap, cnt_SIFTmod)
    plt.plot(temporal_gap, cnt_SP)
    plt.plot(temporal_gap, cnt_IP)

    plt.legend(['SIFT', 'modified SIFT', 'SuperPoint', 'IRPoint'])
    ax.yaxis.set_major_locator(MultipleLocator(0.05))
    plt.minorticks_on()
    plt.grid(which='both')
    plt.title('Temporal Detector Repeatability')
