...
# todo: new descriptor loss - done
def ir_descriptor_loss(descriptors, illum_descriptors,
    ...
    # warped_coord_cells = tf.tile(tf.reshape(coord_cells, [1, Hc, Wc, 2]), [batch_size, 1, 1, 1])
    ...
    # warped_coord_cells = tf.to_float(tf.reshape(warped_coord_cells,
    ...
    illum_descriptors = tf.reshape(illum_descriptors,
                                   [batch_size, 1, 1, Hc, Wc, -1])
    illum_descriptors = tf.nn.l2_normalize(illum_descriptors, -1)
    dot_product_desc = tf.reduce_sum(descriptors * illum_descriptors, -1)
    ...
    loss = config['lambda_d'] * positive_dist
    ...
    tf.summary.scalar('positive_dist', tf.reduce_sum(valid_mask * config['lambda_d'] *
                                                     positive_dist) / normalization)
    tf.summary.scalar('negative_dist', tf.reduce_sum(valid_mask *
