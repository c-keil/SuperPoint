...
def modify_kp(kp):
    if kp.angle > 179:
        angle_new = kp.angle - 180
    else:
        angle_new = kp.angle
    kp_mod = cv2.KeyPoint(kp.pt[0], kp.pt[1], kp.size, angle_new, kp.response, kp.octave, kp.class_id)
    return kp_mod


def modify_desc(desc):
    desc_new = copy.deepcopy(desc)
    for i in range(16):
        # bin = desc[i*8:(i+1)*8]
...
if __name__ == '__main__':
    ...
    for i in range(no_images):
        for j in range(no_images):
            ...
            no_matches_SP[abs(j - i)] += len(match) #/ (min(len(kp_SP[i]), len(kp_SP[j])))
            total[abs(j - i)] += 1
            ...
            no_matches_IP[abs(j - i)] += len(match) #/ (min(len(kp_IP[i]), len(kp_IP[j])))
            ...
            no_matches_SIFT[abs(i - j)] += len(sift_matches) #/ (min(len(kp_SIFT[i]), len(kp_SIFT[j])))
            ...
            no_matches_SIFTmod[abs(i - j)] += len(siftmod_matches) #/ (min(len(kp_SIFTmod[i]), len(kp_SIFTmod[j])))
    ...
    fig.autofmt_xdate()
    plt.plot(temporal_gap[5:], no_matches_SIFT[5:] / total[5:], 'r-', temporal_gap[5:],
             no_matches_SIFTmod[5:] / total[5:], 'c-',temporal_gap[5:], no_matches_SP[5:] / total[5:], 'b-',
             temporal_gap[5:], no_matches_IP[5:] / total[5:], 'g-')
    ...
    plt.minorticks_on()
    plt.grid(which='both')
    plt.title('Average Match Count V Temporal Gap')
    plt.xlabel('Temporal gap (hours)')
    plt.ylabel('Average Match Count')
    plt.show()
